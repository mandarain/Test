...
public class Parse implements Callable {
    ...
    Parse(Element mainElement, Database newDatabase, String forumName) {
    ...
    public Integer call() {
        ...
        //主页面信息存入数据库：
        try {
            newDatabase.putInMainTable(title.attr("href"), forumName, title.text(), url, mainId.text());
        ...
        for (Element cenElement : cenElements) {
            ...
            //子页面信息存入子数据库：
            try {
                newDatabase.putInSubTable(title.attr("href"), forumName, c, cenId.text(), cenContents.text(), imgList);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
