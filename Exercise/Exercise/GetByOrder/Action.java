...
public class Action {
    public static void main(String[] args) throws SQLException, ClassNotFoundException, InterruptedException, ExecutionException, UnsupportedEncodingException {
        //贴吧网页前缀：
        String mainUrl = "https://tieba.baidu.com/f?kw=%E7%81%AB%E5%BD%B1%E5%BF%8D%E8%80%85&ie=utf-8&pn=";
        //贴吧站名：
        ...
        for (int pn = 0; pn < 50; pn = pn + 50) {
            ...
            for (Element mainElement : mainElements) {
                future = pool.submit(new Parse(mainElement, newDatabase, forumName));
            }
        }
        pool.shutdown();
        //判断线程是否结束，并输出数量结果：
